...
If your event destination receives [Organisation](https://docs.stripe.com/get-started/account/orgs.md) events, those requiring a response have the following limitations:

- You can’t subscribe to `issuing_authorization.request` for organisation destinations. Instead, set up a [webhook endpoint](https://docs.stripe.com/webhooks.md#example-endpoint) in a Stripe account within the organisation to subscribe to this event type. Use `issuing_authorization.request` to authorise purchase requests in real-time.
- Organisation destinations receiving `checkout_sessions.completed` can’t [handle redirect behaviour](https://docs.stripe.com/checkout/fulfillment.md#redirect-hosted-checkout) when you embed [Checkout](https://docs.stripe.com/payments/checkout.md) directly in your website or redirect customers to a Stripe-hosted payment page. To influence Checkout redirect behaviour, process this event type with a [webhook endpoint](https://docs.stripe.com/webhooks.md#example-endpoint) configured in a Stripe account within the organisation.
- Organisation destinations responding unsuccessfully to an `invoice.created` event can’t influence [automatic invoice finalisation when using automatic collection](https://docs.stripe.com/billing/subscriptions/webhooks.md#understand). You must process this event type with a [webhook endpoint](https://docs.stripe.com/webhooks.md#example-endpoint) configured in a Stripe account within the organisation to trigger automatic invoice finalisation.
...
- For [organisation](https://docs.stripe.com/get-started/account/orgs.md) event handlers, it inspects the `context` value to determine which account in an organisation generated the event, then sets the `Stripe-Context` header corresponding to the `context` value.
...
This code snippet is a webhook function configured to receive thin events across an organisation, verify the signature, determine the originating account with the `context` field and use that account’s API key for subsequent API calls.
...
> 
> If you create a webhook endpoint in an [organisation account](https://docs.stripe.com/get-started/account/orgs.md), select **Accounts** to listen to events from accounts in your organisation. If you have [Connect platforms](https://docs.stripe.com/connect.md) as members of your organisations and want to listen to events from the all the platforms’ connected accounts, select **Connected accounts**.
