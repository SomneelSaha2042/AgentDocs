...
This module contains functions and types to encode and decode [JWT](https://authjs.dev/concepts/session-strategies#jwt-session)s issued and used by Auth.js.

The JWT issued by Auth.js is *encrypted by default*, using the *A256CBC-HS512* algorithm ([JWE](https://www.rfc-editor.org/rfc/rfc7518.html#section-5.2.5)). It uses the `AUTH_SECRET` environment variable or the passed `secret` property to derive a suitable encryption key.
...
[`Awaitable`](types#awaitablet)<`null` | [`JWT`](jwt#jwt)\>
...
[`Promise`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise)<`R` *extends* `true` ? `string` : `null` | [`JWT`](jwt#jwt)\>

[errors](/reference/core/errors "errors")[providers](/reference/core/providers "providers")
